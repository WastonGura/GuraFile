...
- [x] 原生兼容 v0.2.0 与 v0.3.0 schema v5 数据库，升级无缝。

## 性能与稳定性

- [x] 批量操作与索引提交共享后台写入门，不与 FileSystemWatcher 事件发生死锁或竞争。
- [x] Release x64 自包含构建成功，0 警告、0 错误；224 项自动化测试全部通过。
- [x] 真实窗口启动与响应正常，无残留进程。

## 构建与启动

- [x] 文件版本 `0.3.1.0`、产品版本 `0.3.1` 与目标标签 `v0.3.1` 对齐。
...
- [x] GitHub `v0.3.1` Pre-release 已创建，ZIP 与 SHA-256 附件可回下载并验证。
