...
本版为回收站测试产物精准清理与执行器真实失败路径防护版本，在 v0.3.1 基础上实现测试产物真实无害清理并补齐端到端失败保护实测：

- 真实实现回收站测试产物精准清理（#53）：在测试环境中通过 Shell COM STA 枚举匹配项目并调用还原及彻底删除，杜绝测试运行后的 GUID 项目残留，且绝不影响用户既有文件；
- 补齐真实 Shell / 回收站失败路径端到端测试（#53）：通过排他锁定实测验证真实执行器遇到 Shell 失败时安全报告失败、源文件原位完好无损、SQLite 数据库节点在线状态与用户标签不受任何损坏；
- 保持强制回收站软删除语义（`FOFX_RECYCLEONDELETE` + `FOF_ALLOWUNDO`），严禁任何永久删除降级；
...
1. 从 GitHub Releases 下载 `GuraFile-v0.3.2-win-x64.zip` 和对应的 `.sha256` 文件。
