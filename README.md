...
本版在 v0.1.0 标签文件库基础上增加自动分类与实时一致性：

- 添加一个或多个管理根目录，启动时异步补偿扫描；
...
- 根据扩展名和最多 32 字节文件头生成只读自动类型/格式标签；
- 显示扩展名与文件头冲突，并支持手动重新识别在线文件；
- 跟踪外部创建、修改、删除、重命名和移动，正常负载下目标为 2 秒内更新；
- 监听错误或丢失事件后自动差异恢复，根目录离线时保留节点和标签并在上线后恢复。

这是 Alpha，不建议把 SQLite 索引数据库本身作为唯一备份。重要整理结果请定期使用“导出备份”。

## 安装与运行

1. 从 GitHub Releases 下载 `GuraFile-v0.2.0-win-x64.zip` 和对应的 `.sha256` 文件。
...
7. 自动标签显示在独立列表中，可与用户标签一起按“任一/全部”筛选，但不能被用户改写。
...
- 从 v0.1.0 升级时，数据库会在启动时自动迁移到 schema v5。升级前建议先导出标签；升级后的数据库不能由旧版 GuraFile 打开。
...
```powershell
.\scripts\PackageRelease.ps1 -Version 0.2.0
```
