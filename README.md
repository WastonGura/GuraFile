...
本版在自动分类与实时一致性基础上新增安全文件操作、剪贴板/拖放互通、回收站删除及结构化详情：
...
- 实现删除到 Windows 回收站（`FOF_ALLOWUNDO`），严禁永久删除，删除后保留标签以便恢复；
...
1. 从 GitHub Releases 下载 `GuraFile-v0.3.0-win-x64.zip` 和对应的 `.sha256` 文件。
